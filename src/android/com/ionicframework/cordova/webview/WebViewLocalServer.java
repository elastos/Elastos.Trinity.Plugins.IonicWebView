 ...
 * hosting assets, resources and other data on 'virtual' http(s):// URL. Hosting
 * assets and resources on http(s):// URLs is desirable as it is compatible with
...
public class WebViewLocalServer {
    ...
    public final static String assetScheme = "asset";
    public final static String trinityScheme = "trinity";

    private final UriMatcher uriMatcher;
    private final AndroidProtocolHandler protocolHandler;
    private final String authority;
    private final String customScheme;
    // Whether we're serving local files or proxying (for example, when doing
    // livereload on a
    ...
    public void setFragment(WebViewFragment fragment) {
        ...
        this.appPath = AppManager.getShareInstance().getAppPath(fragment.appInfo);
    }
     ...
     * be taken to correctly synchronize access to any shared state.
     * <p>
     * On Android KitKat and above these methods may be called on more than one
     * thread. This thread may be different than the thread on which the
     * shouldInterceptRequest method was invoke. This means that on Android KitKat
     * and above it is possible to block in this method without blocking other
     * resources from loading. The number of threads used to parallelize loading is
     * an internal implementation detail of the WebView and may change between
     * updates which means that the amount of time spend blocking in this method
    ...
    public abstract static class PathHandler {
        ...
        public PathHandler(String encoding, String charset, int statusCode, String reasonPhrase,
    ...
    private static WebResourceResponse createWebResourceResponse(String mimeType, String encoding, int statusCode,
     ...
     * <code>request</code>. This method should be invoked from within
     * {@link android.webkit.WebViewClient#shouldInterceptRequest(android.webkit.WebView, android.webkit.WebResourceRequest)}.
     *
    ...
    private boolean isLocalFile(Uri uri) {
        String path = uri.getPath();
        if (path.startsWith(contentStart) || path.startsWith(fileStart) || assetScheme.equals(uri.getScheme())) {
            return true;
        }
        return false;
    }
    ...
    private WebResourceResponse handleLocalRequest(Uri uri, PathHandler handler, WebResourceRequest request) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP && request != null
            ...
            return createWebResourceResponse(mimeType, handler.getEncoding(), statusCode, handler.getReasonPhrase(),
                    tempResponseHeaders, responseStream);
        }

        if (isLocalFile(uri)) {
            InputStream responseStream = new LollipopLazyInputStream(handler, uri);
            String mimeType = getMimeType(path, responseStream);
            return createWebResourceResponse(mimeType, handler.getEncoding(), handler.getStatusCode(),
                    handler.getReasonPhrase(), handler.getResponseHeaders(), responseStream);
        }
        ...
        if (path.equals("") || path.equals("/") || (!uri.getLastPathSegment().contains(".") && html5mode)) {
            ...
            try {
                String startPath = launchURL.substring(7);;
                stream = protocolHandler.openFile(startPath);
            } catch (IOException e) {
                Log.e(TAG, "Unable to open " + launchURL, e);
                return null;
            }

            return createWebResourceResponse("text/html", handler.getEncoding(), handler.getStatusCode(),
                    handler.getReasonPhrase(), handler.getResponseHeaders(), stream);
        }
    ...
    private WebResourceResponse handleProxyRequest(Uri uri, PathHandler handler) {
        try {
            ...
            return createWebResourceResponse("", handler.getEncoding(), handler.getStatusCode(),
                    handler.getReasonPhrase(), handler.getResponseHeaders(), stream);
    ...
    /**
     * Registers a handler for the given <code>uri</code>. The <code>handler</code>
     * will be invoked every time the <code>shouldInterceptRequest</code> method of
     * the instance is called with a matching <code>uri</code>.
     *
     * @param uri     the uri to use the handler for. The scheme and authority
     *                (domain) will be matched exactly. The path may contain a '*'
     *                element which will match a single element of a path (so a
     *                handler registered for /a/* will be invoked for /a/b and
     *                /a/c.html but not for /a/b/b) or the '**' element which will
    ...
    /**
     * Hosts the application's assets on an http(s):// URL. Assets from the local
     * path <code>assetPath/...</code> will be available under
     ...
     *                  be made available by the server (for example "/www").
     */
     ...
     *
     * @param domain    custom domain on which the assets should be hosted (for
     *                  example "example.com").
     * @param assetPath the local path in the application's asset folder which will
     *                  be made available by the server (for example "/www").
  ...
  private void createHostingDetails() {
        ...
        try {
          if (path.startsWith(contentStart)) {
            ...
            if (!path.startsWith(fileStart)) {
              path = assetPath + url.getPath();
            }
            stream = protocolHandler.openFile(path);
          }
          else if (assetScheme.equals(url.getScheme())) {
            stream = protocolHandler.openAsset(assetPath + path);
          }
          else {
              stream = protocolHandler.openFile(appPath + path.substring(1));
          }
        } catch (IOException e) {
    ...
    registerUriForScheme(assetScheme, handler, "www");
     ...
     *
     * @param virtualResourcesPath the path on the local server under which the
     ...
     *
     * @param domain               custom domain on which the assets should be
     *                             hosted (for example "example.com"). If untrusted
     *                             content is to be loaded into the WebView it is
     *                             advised to make this random.
     * @param virtualResourcesPath the path on the local server under which the
     *                             resources should be hosted.
     * @param enableHttp           whether to enable hosting using the http scheme.
     * @param enableHttps          whether to enable hosting using the https scheme.
     * @return prefixes under which the resources are hosted.
     */
    public AssetHostingDetails hostResources(final String domain, final String virtualResourcesPath, boolean enableHttp,
            boolean enableHttps) {
    ...
    /**
     * Hosts the application's files on an http(s):// URL. Files from the assetPath
     * <code>assetPath/...</code> will be available under
     * <code>http(s)://{uuid}.androidplatform.net/...</code>.
     *
     * @param assetPath the local path in the application's data folder which will be
     *                 made available by the server (for example "/www").
     */
    public void hostFiles(final String assetPath) {
        this.isAsset = false;
        this.assetPath = assetPath;
        createHostingDetails();
    }
